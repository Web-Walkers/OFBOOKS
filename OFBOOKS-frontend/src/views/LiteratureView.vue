<template>
    <div class="pt-28 px-5 lg:px-16">
        ...
        <div class=" w-full flex-wrap bg-maincolor flex gap-4 shadow-sm rounded-xl text-white text-sm font-['Noto_Kufi_Arabic'] p-4">
            <span class="flex items-center cursor-pointer"><svg class="h-6 lg:hidden inline" @click="categoryToggle()" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M12 3C11.175 3 10.5 3.675 10.5 4.5C10.5 5.325 11.175 6 12 6C12.825 6 13.5 5.325 13.5 4.5C13.5 3.675 12.825 3 12 3ZM12 18C11.175 18 10.5 18.675 10.5 19.5C10.5 20.325 11.175 21 12 21C12.825 21 13.5 20.325 13.5 19.5C13.5 18.675 12.825 18 12 18ZM12 10.5C11.175 10.5 10.5 11.175 10.5 12C10.5 12.825 11.175 13.5 12 13.5C12.825 13.5 13.5 12.825 13.5 12C13.5 11.175 12.825 10.5 12 10.5Z"></path></svg> ادبیات ملل : </span>
            <RouterLink class="px-3 py-2 rounded-2xl border border-white lg:inline" :class="(isToggleCategory ? 'inline animate-fade-down' : 'hidden') + ( query.literature_id === literature.id ? ' bg-white text-black' : '')"
				v-for="literature in literatures" v-bind:key="literature.id"
				:to="{name: 'literature', params: {id: literature.id}}"
				>{{ literature.name }}</RouterLink>
        </div>
        <div class="flex flex-col md:flex-row my-4 py-2 px-4 items-center md:gap-10 lg:mx-20 lg:px-5 lg:py-5 lg:my-5 rounded-lg border border-gray-100 shadow-md" v-if="current_literature != null">
            <div class=" shrink-0"><img class="h-56 rounded-full border-4 border-maincolor" :src="current_literature.get_image" alt=""></div>
            <div class="">
                <h6 class=" text-xl font-semibold">{{ current_literature.name }}</h6>
                <p class=" text-">{{ current_literature.description }}</p>
            </div>
        </div>
        ...
        <div class=" w-full flex-wrap bg-maincolor flex gap-4 shadow-sm rounded-xl text-white text-sm font-['Noto_Kufi_Arabic'] p-4">
            <div class="flex items-center md:px-6 " >
                <form class="relative text-gray-600 block " v-on:submit.prevent="search">
                    <input type="search" name="serch" placeholder="جستجو" class=" h-10 w-full md:w-96 md:pr-8 md:pl-5 bg-transparent rounded-xl z-0 placeholder:text-white transition-all duration-500 outline-none border-gray-50 focus:border-gray-100 focus:outline-none focus:ring-0 text-white" v-model="query.keywords">
...
export default {
	...
	watch: {
        '$route.params.id': {
            handler: function() {
				this.query.literature_id = this.$route.params.id === 'all' ? '' : this.$route.params.id
	...
	methods: {
		...
		getLiterature() {
			axios
				.get(`/api/books/literature/${this.$route.params.id}/`)
				.then(response => {
					this.current_literature = response.data
				})
				.catch(error => {
					console.log('error', error)
				})
