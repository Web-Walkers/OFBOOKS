        ...
        <div class="px-5 lg:px-16">   
            ...
            <div class="px-5 lg:px-28 pt-10">
                <ol class="flex items-center w-full text-sm font-medium text-center text-gray-500 dark:text-gray-400 sm:text-base">
                    <li class="flex md:w-full text-maincolor items-center sm:after:content-[''] after:w-full after:h-1 after:border-b after:border-gray-200 after:border-1 after:hidden sm:after:inline-block after:mx-6 xl:after:mx-10 dark:after:border-gray-700">
            ...
            <div class="relative rounded-md overflow-x-auto max-w-7xl mx-auto pt-8 shadow-lg my-3 ">
                    ...
                    <tbody>
                        ...
                        >
                            <td class="p-2 md:p-4 w-28 hidden md:inline-block">
                                <RouterLink :to="get_url(item.product_type, item.product_object.id)">
                                    <img  v-bind:src="item.product_object.get_image" class="w-16 md:w-32 max-w-full max-h-full" :alt="item.product_object.name">
                                </RouterLink>
                            </td>
                            ...
                            <td class=" px-2 md:px-6 py-4 font-semibold text-gray-900">
                                {{ item.product_object.readable_credit }} تومان
                            </td>
                            <td class="px-2 md:px-6 py-4">
                                <button href="#" class="font-medium text-red-600 dark:text-red-500 hover:underline" @click="() => setQuantity(-1, item.id)">حذف</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="px-5 py-8 space-y-2">
                    <div class="flex gap-4 justify-between"><p> جمع کل خرید :</p> <span>{{ cart.get_readable_credit }} تومان</span></div>
                    <div class="flex gap-4 justify-between text-red-950 "><p> مبلغ تخفیف :</p> <span>000 تومان</span></div>
                    <div class="flex gap-4 justify-between text-maincolor"><p> مبلغ قابل پرداخت :</p> <span>{{ cart.get_readable_credit }} تومان</span></div>
                </div>
                <div class="p-4 flex justify-end"><RouterLink class="bg-maincolor text-white py-2 px-4 rounded-lg" :to="{name:'shipping', params: {id: cart.id}}">ثبت سفارش</RouterLink></div>
            </div>
...
export default {
    ...
    methods: {
        ...
        get_url(type, id) {
            if (type === 11) 
            {
                return {name: 'product', params: {'id': id}}
            } 
            else if ( type === 16 ) 
            {
                return {name: 'book', params: {'id': id}}
            }
            else 
            {
                return {name: 'home'}
