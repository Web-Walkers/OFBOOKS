<template>
    <div class=" pt-28">
        <div class=" w-full flex-wrap bg-maincolor flex gap-4 shadow-sm rounded-xl text-white text-sm font-['Noto_Kufi_Arabic'] p-4" >
            <div class="w-full flex-wrap flex gap-4 text-sm px-4">
                <span class="flex items-center cursor-pointer"><svg class="h-6 lg:hidden inline" @click="categoryToggle()" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M12 3C11.175 3 10.5 3.675 10.5 4.5C10.5 5.325 11.175 6 12 6C12.825 6 13.5 5.325 13.5 4.5C13.5 3.675 12.825 3 12 3ZM12 18C11.175 18 10.5 18.675 10.5 19.5C10.5 20.325 11.175 21 12 21C12.825 21 13.5 20.325 13.5 19.5C13.5 18.675 12.825 18 12 18ZM12 10.5C11.175 10.5 10.5 11.175 10.5 12C10.5 12.825 11.175 13.5 12 13.5C12.825 13.5 13.5 12.825 13.5 12C13.5 11.175 12.825 10.5 12 10.5Z"></path></svg> هشتگ ها  : </span>
                <RouterLink class="px-3 py-2 rounded-2xl border border-white lg:inline" :class="(isToggleCategory ? 'inline animate-fade-down' : 'hidden') + ( query.tag_id === '' ? ' bg-white text-black' : '')" 
                    :to="{name: 'blogs', params: {id: 'all'}}"
                    >جستجو در همه</RouterLink>
                <RouterLink class="px-3 py-2 rounded-2xl border border-white lg:inline" :class="(isToggleCategory ? 'inline animate-fade-down' : 'hidden') + ( query.tag_id === tag.id ? ' bg-white text-black' : '')"
                    v-for="tag in tags" v-bind:key="tag.id"
                    :to="{name: 'blogs', params: {id: tag.id}}"
                    >#{{ tag.name }}</RouterLink>
            </div>
        </div>
			...
			<div class="flex items-center md:px-6 ">
				<form class="relative text-gray-600 block " v-on:submit.prevent="search">
					<input type="search" name="serch" placeholder="جستجو"
						class=" h-10 w-full md:w-96 md:pr-8 md:pl-5 bg-transparent rounded-xl z-0 placeholder:text-white transition-all duration-500 outline-none border-gray-50 focus:border-gray-100 focus:outline-none focus:ring-0 text-white"
        ...
        <div class="w-full flex-wrap items-center flex gap-4 shadow-sm rounded-xl text-maincolor text-sm px-10 py-2 pb-10">
            ...
            <button class="px-3 py-2 rounded-2xl border-maincolor lg:inline" :class="(isToggleSearchSort ? 'inline animate-fade-down' : 'hidden') + (query.sort_by === 'view' ? ' bg-maincolor text-white' : '')" @click="() => sort('view')">پربازدید ترین ها</button>
            <button class="px-3 py-2 rounded-2xl border-maincolor lg:inline" :class="(isToggleSearchSort ? 'inline animate-fade-down' : 'hidden') + (query.sort_by === 'popular' ? ' bg-maincolor text-white' : '')" @click="() => sort('popular')">محبوب ترین ها</button>
            <button class="px-3 py-2 rounded-2xl border-maincolor lg:inline" :class="(isToggleSearchSort ? 'inline animate-fade-down' : 'hidden') + (query.sort_by === 'new' ? ' bg-maincolor text-white' : '')" @click="() => sort('new')">جدید ترین ها</button>
        </div>       
...
export default {
    ...
    watch: {
        '$route.params.id': {
            handler: function() {
				this.query.tag_id = this.$route.params.id === 'all' ? '' : this.$route.params.id
    ...
    methods: {
        getBlogs() {
            axios
                .post('/api/blogs/', this.query)
                .then(response => {
                    this.blogs = response.data
                    this.query.idx = this.blogs.length
