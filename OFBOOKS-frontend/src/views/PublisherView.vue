<template>
    <div class="pt-28 px-5 lg:px-16">
        ...
        <div class=" w-full flex-wrap bg-maincolor flex gap-4 shadow-sm rounded-xl text-white text-sm font-['Noto_Kufi_Arabic'] p-4">
            ...
            <RouterLink class="px-3 py-2 rounded-2xl border border-white lg:inline" :class="(isToggleCategory ? 'inline animate-fade-down' : 'hidden') + ( query.publisher_id === '' ? ' bg-white text-black' : '')" 
				:to="{name: 'publisher', params: {id: 'all'}}"
				>همه محصولات</RouterLink>
            <RouterLink class="px-3 py-2 rounded-2xl border border-white lg:inline" :class="(isToggleCategory ? 'inline animate-fade-down' : 'hidden') + ( query.publisher_id === publisher.id ? ' bg-white text-black' : '')"
				v-for="publisher in publishers" v-bind:key="publisher.id"
				:to="{name: 'publisher', params: {id: publisher.id}}"
				>{{ publisher.name }}</RouterLink>
        </div>
        <div class="flex flex-col md:flex-row my-4 py-2 px-4 items-center md:gap-10 lg:mx-20 lg:px-5 lg:py-5 lg:my-5 rounded-lg border border-gray-100 shadow-md" v-if="current_publisher != null">
            <div class=" shrink-0"><img class="h-56 rounded-full border-4 border-maincolor" :src="current_publisher.get_image" alt=""></div>
            <div class="">
                <h6 class=" text-xl font-semibold">{{ current_publisher.name }}</h6>
                <p class=" text-">{{ current_publisher.description }}</p>
            </div>
        </div>
		<div class="flex flex-col md:flex-row my-4 py-2 px-4 items-center md:gap-10 lg:mx-20 lg:px-5 lg:py-5 lg:my-5 rounded-lg border border-gray-100 shadow-md" v-else>
			<!-- TODO -->
        ...
        <div class=" w-full flex-wrap bg-maincolor flex gap-4 shadow-sm rounded-xl text-white text-sm font-['Noto_Kufi_Arabic'] p-4">
            <div class="flex items-center md:px-6 " >
                <form class="relative text-gray-600 block " v-on:submit.prevent="search">
                    <input type="search" name="serch" placeholder="جستجو" class=" h-10 w-full md:w-96 md:pr-8 md:pl-5 bg-transparent rounded-xl z-0 placeholder:text-white transition-all duration-500 outline-none border-gray-50 focus:border-gray-100 focus:outline-none focus:ring-0" v-model="query.keywords">
                    <button type="submit" class="absolute left-0 inset-y-0 ml-4 flex items-center justify-center text-white">
                        <svg class=" h-6 w-6 " xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M18.031 16.6168L22.3137 20.8995L20.8995 22.3137L16.6168 18.031C15.0769 19.263 13.124 20 11 20C6.032 20 2 15.968 2 11C2 6.032 6.032 2 11 2C15.968 2 20 6.032 20 11C20 13.124 19.263 15.0769 18.031 16.6168ZM16.0247 15.8748C17.2475 14.6146 18 12.8956 18 11C18 7.1325 14.8675 4 11 4C7.1325 4 4 7.1325 4 11C4 14.8675 7.1325 18 11 18C12.8956 18 14.6146 17.2475 15.8748 16.0247L16.0247 15.8748Z"></path></svg>
                    </button>
                </form>
            </div>
        </div>
        <div class="w-full flex-wrap items-center flex gap-4 shadow-sm rounded-xl text-maincolor text-sm px-10 py-2 pb-10">
...
export default {
	...
	watch: {
        '$route.params.id': {
            handler: function() {
				this.query.publisher_id = this.$route.params.id === 'all' ? '' : this.$route.params.id
	...
	methods: {
		...
		getPublisher() {
			axios
				.get(`/api/books/publisher/${this.$route.params.id}/`)
				.then(response => {
					this.current_publisher = response.data
				})
				.catch(error => {
					console.log('error', error)
				})
